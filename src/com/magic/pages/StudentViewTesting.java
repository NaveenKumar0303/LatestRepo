...
public class StudentViewTesting {
		...
		try
		{
			...
			for(int i=1;i<=elements.size();i++)
			{
				...
				if(Selenium.getWebElements(By.xpath(xpathLongdesc+"["+i+"]/ancestor::figure//following-sibling::a//img[@class='longdesc-icon']")).size()>0)
				{
					...
					String longdescText = Selenium.getWebElement(By.xpath("//body")).getText();

					longdesc_list.add(pageHeading+"@#"+page+"@#"+longdescEle.getAttribute("outerHTML")+"@#"+longdescText);
	...
	private void validationFontSize(String heading, String page) {
